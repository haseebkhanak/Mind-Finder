<?php
 ...
 {   
     ...
     if ($_SERVER['REQUEST_METHOD'] === 'POST') 
     {
         ...
         $sql="UPDATE ca_result set Score='$score_2' where Email='$email'";
         $result=mysqli_query($conn,$sql);

         if ($result)
         {
         $sql = "INSERT INTO ca_result VALUES ( '$email','$score_2')";
         $result = mysqli_query($conn, $sql);
 
         if ($result) 
         {
            
             $sql = "SELECT details.Id, details.Name, ca_result.Score FROM details INNER JOIN ca_result ON details.Email = ca_result.email
