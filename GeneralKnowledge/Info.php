<?php
 ...
 {   
     ...
     if ($_SERVER['REQUEST_METHOD'] === 'POST') 
     {
         ...
         $sql="UPDATE gk_result set Score='$score_4' where Email='$email'";
         $result=mysqli_query($conn,$sql);

         if ($result)
         {
         $sql = "INSERT INTO gk_result VALUES ( '$email','$score_4')";
         $result = mysqli_query($conn, $sql);
 
         if ($result) 
         {
            
             $sql = "SELECT details.Id, details.Name, gk_result.Score FROM details INNER JOIN gk_result ON details.Email = gk_result.email
